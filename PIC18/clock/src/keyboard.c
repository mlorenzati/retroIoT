...
char repeat_rate_30ms;
char repeat_rate_30ms_partial;
char repeat_rate_30ms_fast;
...
void keyboard_handler(void) {
    ...
    if (value != stat->pressed) {
        if (value && stat->count == KEYBOARD_THRESHOLD) {
            stat->pressed = true;
            stat->repeating = false;
            key_changed = true;

            // Prepare the repeat key, refresh rate is 5ms * 6 keys (30ms)
            stat->repeat_countdown = repeat_rate_30ms;
    ...
    if (stat->repeat_countdown > 0) {
        if (--(stat->repeat_countdown) == 0) {
            // Key still pressed repeat countdown, now faster
            stat->repeating = true;
            key_changed = true;
            stat->repeat_countdown = repeat_rate_30ms_fast;
        }
    }
...
void keyboard_init(const char *key_idx, char _repeat_rate_30ms, char _repeat_rate_30ms_fast, keyboard_callback callback) {
    // Configure I/O Ports
